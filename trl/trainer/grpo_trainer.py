...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if self.enable_think_revise:
            # Validate oversampling: need more BM25 results than final MMR results
            if args.retriever_top_k < args.memory_top_m:
                logger.warning(
                    f"retriever_top_k ({args.retriever_top_k}) < memory_top_m ({args.memory_top_m}). "
                    f"For effective MMR diversity, retriever_top_k should be larger (e.g., 2-3x memory_top_m)."
                )
            
            # Pass deduplication function to retriever for database-level dedup
            dedup_fn = lambda a, b: jaccard_ngrams(a, b, n=3)
            self.retriever = InMemoryBM25Temporal(
                dedup_threshold=args.dedup_jaccard,
                dedup_sim_fn=dedup_fn
            )
            self._dist_retr = DistributedRetriever(self.retriever, self.accelerator, args.memory_namespace)
            # Store namespace for retrieval queries
            self.memory_namespace = args.memory_namespace
            # MMR parameters for diversifying retrieval results
            self.retrieval_mmr_k = args.memory_top_m
            self.retrieval_mmr_alpha = args.mmr_alpha
...
<|im_start|>assistant\n<actions>\n"""
    ...
    def _get_per_token_logps_and_entropies(
        ...
        for start in range(0, input_ids.size(0), batch_size):
            ...
            if apply_temperature:
                # Handle NaN/Inf in logits before division to prevent CUDA errors
                # Use clamp which is more stable than nan_to_num on CUDA
                torch.cuda.synchronize()  # Sync to catch any prior CUDA errors
                
                # Check for NaN values and raise an error if found (indicates model instability)
                if torch.isnan(logits).any():
                    raise ValueError(
                        "NaN values detected in logits. This indicates model instability. "
                        "Consider: (1) reducing learning rate, (2) enabling gradient clipping, "
                        "(3) checking for corrupted data, or (4) using mixed precision training."
                    )
    ...
    def _generate_and_score_completions(
        ...
        if self.enable_think_revise:
            ...
            # Step 1: Oversample from BM25 to get a larger pool of candidates
            queries = think_texts
            hits_lists = self._dist_retr.query_batch(
                queries,
                cutoff_ts_list=now_tss,
                top_k=self.args.retriever_top_k,  # Fetch MORE than we need (e.g., 5)
                # TODO: decay is generated by the model itself!
                time_decay_lambda=self.args.time_decay_lambda,
                namespaces=[self.memory_namespace],
            )
            
            # Step 2: Apply MMR to select diverse subset from the pool
            # This balances relevance (BM25 score) with diversity (avoiding similar results)
            mmr_hits_lists = []
            for hits in hits_lists:
                if hits:
                    # Convert hits to MMR format: (text, utility, payload)
                    items = [(h['text'], h['score'], h) for h in hits]
                    # Select diverse subset using MMR (e.g., keep best 3 out of 5)
                    selected = mmr_select(
                        items, 
                        sim_fn=lambda a, b: jaccard_ngrams(a, b, n=3),
                        top_m=self.retrieval_mmr_k,  # Final count (e.g., 3)
                        alpha=self.retrieval_mmr_alpha  # 0.7 = 70% relevance, 30% diversity
                    )
                    mmr_hits_lists.append([item[2] for item in selected])  # extract original hits
                else:
                    mmr_hits_lists.append([])
            
            retrieved_txts = ["\n".join(f"{h['text']}" for h in hits) for hits in mmr_hits_lists]
        ...
        if self.enable_think_revise and self._dist_retr is not None:
            ...
            self._dist_retr.add_candidates(local_rows, visible_delay=1)
